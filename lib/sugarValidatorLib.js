function validate(fileData, localStorage = {}) {
  ...
  function matchHTML(html, passage) {
      const tags = ["span", "div", "center", "b", "i", "a", "video", "style", "h1", "h2", "h3", "h4", "h5", "h6", "p", 
    "head", "body", "audio"];
  
      tags.forEach(tag => {
        const openTag = new RegExp(`<${tag}(\\s|>)`, "g");
        const closeTag = new RegExp(`</${tag}>`, "g");
        const numOpen = (html.match(openTag) || []).length;
        const numClose = (html.match(closeTag) || []).length;
    
        if (numOpen > numClose) addWarning([[`Unmatched <${tag}> in passage`]], passage.header)
        else if (numOpen < numClose) addWarning([[`Unmatched </${tag}> in passage`]], passage.header);
      });
  };
