function validate(fileData, localStorage = {}) {
    ...
    function matchPairs(html, passage) {   // using warnings in case someone uses old school smileys in their code :)
        ...
        if (numCurlyBraces[0] > numCurlyBraces[1]) addWarning([['More opening than closing curly braces in passage']], passage.header)
        else if (numCurlyBraces[0] < numCurlyBraces[1]) addWarning([['More closing than opening curly braces in passage']], passage.header);
    }

    function matchHTML(html, passage) {   // basic version to validate HTML, doesn't return errors because it doesn't look at structure
        const numSpan = [html.split("<span").length - 1, html.split("</span>").length - 1];
        if (numSpan[0] > numSpan[1]) addWarning([['Unmatched <span> in passage']], passage.header)
        else if (numSpan[0] < numSpan[1]) addWarning([['Unmatched </span> in passage']], passage.header);
        const numDiv = [html.split("<div").length - 1, html.split("</div>").length - 1];
        if (numDiv[0] > numDiv[1]) addWarning([['Unmatched <div> in passage']], passage.header)
        else if (numDiv[0] < numDiv[1]) addWarning([['Unmatched </div> in passage']], passage.header);
        const numCenter = [html.split("<center").length - 1, html.split("</center>").length - 1];
        if (numCenter[0] > numCenter[1]) addWarning([['Unmatched <center> in passage']], passage.header)
        else if (numCenter[0] < numCenter[1]) addWarning([['Unmatched </center> in passage']], passage.header);
        const numB = [(html.split("<b>").length + html.split("<b ").length - 2), html.split("</b>").length - 1];
        if (numB[0] > numB[1]) addWarning([['Unmatched <b> in passage']], passage.header)
        else if (numB[0] < num[1]) addWarning([['Unmatched </b> in passage']], passage.header);
        const numI = [(html.split("<i>").length + html.split("<i ").length - 2), html.split("</i>").length - 1];
        if (numI[0] > numI[1]) addWarning([['Unmatched <i> in passage']], passage.header)
        else if (numI[0] < numI[1]) addWarning([['Unmatched </i> in passage']], passage.header);
        const numA = [(html.split("<a>").length + html.split("<a ").length - 2), html.split("</a>").length - 1];
        if (numA[0] > numA[1]) addWarning([['Unmatched <a> in passage']], passage.header)
        else if (numA[0] < numA[1]) addWarning([['Unmatched </a> in passage']], passage.header);
        const numVideo = [html.split("<video").length - 1, html.split("</video>").length - 1];
        if (numVideo[0] > numVideo[1]) addWarning([['Unmatched <video> in passage']], passage.header)
        else if (numVideo[0] < numVideo[1]) addWarning([['Unmatched </video> in passage']], passage.header);
        const numImg = [html.split("<img").length - 1, html.split("</img>").length - 1];
        if (numImg[0] > numImg[1]) addWarning([['Unmatched <img> in passage']], passage.header)
        else if (numImg[0] < numImg[1]) addWarning([['Unmatched </img> in passage']], passage.header);
        const numStyle = [html.split("<style").length - 1, html.split("</style>").length - 1];
        if (numStyle[0] > numStyle[1]) addWarning([['Unmatched <style> in passage']], passage.header)
        else if (numStyle[0] < numStyle[1]) addWarning([['Unmatched </style> in passage']], passage.header);
        const numH1 = [html.split("<h1").length - 1, html.split("</h1>").length - 1];
        if (numH1[0] > numH1[1]) addWarning([['Unmatched <h1> in passage']], passage.header)
        else if (numH1[0] < numH1[1]) addWarning([['Unmatched </h1> in passage']], passage.header);
        const numH2 = [html.split("<h2").length - 1, html.split("</h2>").length - 1];
        if (numH2[0] > numH2[1]) addWarning([['Unmatched <h2> in passage']], passage.header)
        else if (numH2[0] < numH2[1]) addWarning([['Unmatched </h2> in passage']], passage.header);
        const numH3 = [html.split("<h3").length - 1, html.split("</h3>").length - 1];
        if (numH3[0] > numH3[1]) addWarning([['Unmatched <h3> in passage']], passage.header)
        else if (numH3[0] < numH3[1]) addWarning([['Unmatched </h3> in passage']], passage.header);
        const numH4 = [html.split("<h4").length - 1, html.split("</h4>").length - 1];
        if (numH4[0] > numH4[1]) addWarning([['Unmatched <h4> in passage']], passage.header)
        else if (numH4[0] < numH4[1]) addWarning([['Unmatched </h4> in passage']], passage.header);
        const numH5 = [html.split("<h5").length - 1, html.split("</h5>").length - 1];
        if (numH5[0] > numH5[1]) addWarning([['Unmatched <h5> in passage']], passage.header)
        else if (numH5[0] < numH5[1]) addWarning([['Unmatched </h5> in passage']], passage.header);
        const numH6 = [html.split("<h6").length - 1, html.split("</h6>").length - 1];
        if (numH6[0] > numH6[1]) addWarning([['Unmatched <h6> in passage']], passage.header)
        else if (numH6[0] < numH6[1]) addWarning([['Unmatched </h6> in passage']], passage.header);
        const numP = [(html.split("<p>").length + html.split("<p ").length - 2), html.split("</p>").length - 1];
        if (numP[0] > numP[1]) addWarning([['Unmatched <p> in passage']], passage.header)
        else if (numP[0] < numP[1]) addWarning([['Unmatched </p> in passage']], passage.header);
        const numHead = [html.split("<head").length - 1, html.split("</head>").length - 1];
        if (numHead[0] > numHead[1]) addWarning([['Unmatched <head> in passage']], passage.header)
        else if (numHead[0] < numHead[1]) addWarning([['Unmatched </head> in passage']], passage.header);
        const numBody = [html.split("<body").length - 1, html.split("</body").length - 1];
        if (numBody[0] > numBody[1]) addWarning([['Unmatched <body> in passage']], passage.header)
        else if (numBody[0] < numBody[1]) addWarning([['Unmatched </body> in passage']], passage.header);
        const numAudio = [html.split("<audio").length - 1, html.split("</audio>").length - 1];
        if (numAudio[0] > numAudio[1]) addWarning([['Unmatched <audio> in passage']], passage.header)
        else if (numAudio[0] < numAudio[1]) addWarning([['Unmatched </audio> in passage']], passage.header);
    }
        ...
        try {
            ...
            if (!tags.includes('no-scan')) {
                if (!tags.includes('no-warnings')) {
                    ...
                    matchHTML(passage.content, passage);
                }
