function validate(fileData, localStorage = {}) {
    ...
    function matchPairs(html, passage) {   // using warnings in case someone uses old school smileys in their code :)
        const numParrenthesis = [html.split("(").length - 1, html.split(")").length - 1];
        if (numParrenthesis[0] > numParrenthesis[1]) addWarning([['More opening than closing parrenthesis in passage']], passage.header)
        else if (numParrenthesis[0] < numParrenthesis[1]) addWarning([['More closing than opening parrenthesis in passage']], passage.header);
