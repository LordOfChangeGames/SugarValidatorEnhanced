...
function validate(fileData, localStorage = {}) {
    ...
    function findInvalidConditions(html, passage) {
        ...
        if (matches) {addError(`Invalid condition found: '${matches[0]}', is/isnot should not be used in combination with lt/lte/gt/gte`, passage)};
    }

    function findInvalidSetters(html, passage) {
        const matching = html.match(/<<set [^>]* (==|===) [^>]*>>/g);
        if (matching) {addError(`Invalid setter found: '${matches[0]}', ==/=== should not be used in a <<set>>, use to or = instead`, passage)};
    }
