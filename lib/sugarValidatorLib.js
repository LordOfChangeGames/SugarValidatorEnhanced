function validate(fileData, localStorage = {}) {
    ...
    function matchHTML(html, passage) {
      const tagPattern = /<([^\/\s>]+)(\s[^>]*)?>|<\/([^>]+)>/g;
      const selfClosingTags = new Set(["area", "base", "br", "col", "command", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr"]);
      const tagStack = [];
      let match;
      while ((match = tagPattern.exec(html)) !== null) {
        ...
        if (openTag) {
          ...
          if (tagStack.length === 0 || tagStack[tagStack.length - 1].toLowerCase() !== closeTag.toLowerCase()) {
            addWarning([`Unmatched </${closeTag}> in passage`], passage.header);
          } else {
            tagStack.pop();
          }
        }
      ...
      tagStack.forEach(tag => {
        addWarning([`Unmatched <${tag}> in passage`], passage.header);
      });
    }
    
    function addWarning(message, header) {
      // Your existing logic to add warnings
